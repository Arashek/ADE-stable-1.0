...
class ModelTrainer:
        ...
        try:
            for component, config in self.config.components.items():
                ...
                results = await self.evaluation_manager.run_ab_test(
                    component=component,
                    model_a=str(config["output_dir"] / "v1"),
                    model_b=str(config["output_dir"] / "v2"),
                    test_data=self._load_test_data(component)
                )
    ...
    async def export_metrics(self):
        ...
        try:
            ...
            self.training_manager.export_metrics(self.config.metrics_dir / "training")
            
            # Export performance metrics
            self.performance_manager.export_metrics(self.config.metrics_dir / "performance")
            
            # Export evaluation metrics
            self.evaluation_manager.export_metrics(self.config.metrics_dir / "evaluation")
    ...
    async def train_single_component(self, component: str):
